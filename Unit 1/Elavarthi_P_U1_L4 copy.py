...
def a_star(start, goal="_123456789ABCDEF", heuristic=dist_heuristic, size = 4):
   ...
   while frontier.isEmpty() == False:
      ...
      if state[1] not in state[2]:
         ...
         if state[1] == goal:
            return path
         for a in generate_children(state[1], size):
            if a not in path or (explored[state[1]] + 1 + heuristic(a) < explored[a]+heuristic(a)):
               explored[a] = len(path)+1
               frontier.push((explored[a] + heuristic(a), a, path))      
   return None
